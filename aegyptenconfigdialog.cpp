...
#include "aegyptenconfigdialog.h"
...
namespace Aegypten {
  ...
  PluginConfigDialog::PluginConfigDialog( CryptPlugWrapper * wrapper,
  ...
  void PluginConfigDialog::slotOk() {
    assert( mWrapper );
...
*/
  ...
  void PluginConfigDialog::setPluginInformation() {
    assert( mWrapper );
    ...
    for( int i = 0;
	 i < mSignatureTab->signatureAlgorithmCO->count(); ++i )
    ...
    for( int i = 0;
	 i < mEncryptionTab->encryptionAlgorithmCO->count(); ++i )
      if( mEncryptionTab->encryptionAlgorithmCO->text( i ) ==
	  encAlgoStr ) {
  ...
  void PluginConfigDialog::slotStartCertManager() {
    assert( mWrapper );
...
#include "aegyptenconfigdialog.moc"
