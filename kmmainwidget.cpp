...
void KMMainWidget::slotSaveTextAs() {
  if ( mMsgView )
    mMsgView->slotSaveTextAs();
}

//-----------------------------------------------------------------------------
void KMMainWidget::slotOpenMsg()
{
  KMOpenMsgCommand *openCommand = new KMOpenMsgCommand( this, overrideEncoding() );
...
void KMMainWidget::slotReplaceMsgByUnencryptedVersion()
{
  ...
  if( oldMsg ) {
    ...
    if( oldMsg->hasUnencryptedMsg() ) {
      ...
      QString newMsgId( oldMsg->msgId() );
      {
        QString prefix("DecryptedMsg.");
        int oldIdx = newMsgId.find(prefix, 0, false);
        if( -1 == oldIdx ) {
          int leftAngle = newMsgId.findRev( '<' );
          newMsgId = newMsgId.insert( (-1 == leftAngle) ? 0 : ++leftAngle, prefix );
        }
        else {
          // toggle between "DecryptedMsg." and "DeCryptedMsg."
          // to avoid same message id
          QCharRef c = newMsgId[ oldIdx+2 ];
          if( 'C' == c )
            c = 'c';
          else
            c = 'C';
...
void KMMainWidget::setupActions()
{
  ...
  mSaveAsAction = new KAction( i18n("Save Message &As..."), "filesave",
    KStdAccel::shortcut(KStdAccel::Save),
    this, SLOT(slotSaveMsg()), actionCollection(), "file_save_as" );

  mSaveTextAsAction = new KAction( i18n("&Save Text As..."), "filesave", 0,
				   this, SLOT(slotSaveTextAs()),
				   actionCollection(), "file_save_text_as" );
