...
void SnippetWidget::slotAdd()
{
  ...
  if ( dlg.exec() == QDialog::Accepted ) {
    ...
    _list.append( makeItem( group, dlg.snippetName->text(), dlg.snippetText->text(),
                            dlg.shortcutButton->shortcut() ) );
  }
}
 ...
 */
SnippetItem *SnippetWidget::makeItem( SnippetItem *parent, const QString &name, const QString &text,
...
void SnippetWidget::slotEdit( QListViewItem *item )
{
  ...
  if ( dlg.exec() == QDialog::Accepted ) {
    ...
    if ( SnippetItem::findGroupById( pSnippet->getParent(), _list )->getName() != dlg.cbGroup->currentText() ) {
      SnippetGroup *newGroup = dynamic_cast<SnippetGroup*>(
        SnippetItem::findItemByName( dlg.cbGroup->currentText(), _list ) );
...
void SnippetWidget::writeConfig()
{
  ...
  _cfg->deleteGroup( "SnippetPart" ); //otherwise delete entries will stay in list until
  ...
  while ( item != 0 ) {
    ...
    if ( group ) {
      //kdDebug(5006) << "-->GROUP "
      //              << item->getName() << group->getId() << " " << iGroupCount<< endl;
      ...
      //kdDebug(5006) << "-->ITEM "
      //              << item->getName() << item->getParent() << " " << iSnipCount << endl;
  ...
  for ( it = _mapSaved.begin(); it != _mapSaved.end(); ++it ) {  //write the saved variable values
    if ( it.data().length() <= 0 ) {  //is the saved value has no length -> no need to save it
      continue;
    }
  ...
  if ( iCount != -1 ) {
    ...
    for ( int i=0; i<iCount; i++ ) {  //read the snippet-list
      ...
      if ( strNameVal != "" && strTextVal != "" && iParentVal != -1 ) {
        KShortcut shortcut(
          _cfg->readEntry( QString( "snippetShortcut_%1" ).arg( i ), QString() ) );
        item = makeItem( SnippetItem::findGroupById(
                           iParentVal, _list ), strNameVal, strTextVal, shortcut );
...
QString SnippetWidget::parseText( QString text, QString del )
{
  ...
  do {
    iFound = text.find( QRegExp( "\\" + del + "[A-Za-z-_0-9\\s]*\\" + del ), iEnd + 1 );
    if ( iFound >= 0 ) {
  ...
  if ( iInMeth == 1 ) {  //check config, if input-method "multi" is selected
    ...
    if ( showMultiVarDialog( &mapVar, &_mapSaved, w, bh, oh ) ) {  //generate and show the dialog
      ...
      for ( it = mapVar.begin(); it != mapVar.end(); ++it ) {
        //walk through the map and do the replacement
                                        ...
                                        int &iWidth, int &iBasicHeight, int &iOneHeight )
{
  ...
  if ( map->count() == 1 &&
       it.data() == _SnippetConfig.getDelimiter() + _SnippetConfig.getDelimiter() ) {
    return true;
  }

  QMap<QString, KTextEdit *> mapVar2Te;  // track which TEXTEDIT goes with which variable
  QMap<QString, QCheckBox *> mapVar2Cb;  // track which CHECKBOX goes with which variable
  ...
  for ( it = map->begin(); it != map->end(); ++it ) {
    ...
    QToolTip::add(
      cb,
      i18n( "Enable this to save the value entered to the right "
            "as the default value for this variable" ) );
    QWhatsThis::add(
      cb,
      i18n( "If you enable this option, the value entered to the right will be saved. "
