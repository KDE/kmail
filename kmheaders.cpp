  ...
  K3ListView( parent )
{
  ...
  if (!pixmapsLoaded)
  {
    ...
    pixFullySigned           = new QPixmap( SmallIcon( "mail-signed"                ) );
...
void KMHeaders::msgAdded(int id)
{
  ...
  if ( isThreaded() ) {
    ...
    if ( !msgId.isEmpty() ) {
      foreach ( HeaderItem *cur, mImperfectlyThreadedList ) {
        int tryMe = cur->msgId();
...
void KMHeaders::msgRemoved(int id, const QString &msgId )
{
  ...
  if (isThreaded() && mFolder->count()) {
    ...
    foreach ( Q3ListViewItem *lvi, childList ) {
      ...
      if ((!parent || sci->isImperfectlyThreaded())
                      && ! mImperfectlyThreadedList.contains( item ))
...
{
  foreach ( Q3ListViewItem *lvi, currentThread() ) {
  ...
  if ( lvi && e->button() == Qt::LeftButton &&
    ...
    if ( section == mPaintInfo.flagCol && flagsToggleable ) {
      ...
      if ( msg->status().isWatched() || msg->status().isIgnored() )
        ...
        setMsgStatus( KPIM::MessageStatus::statusWatched(), true );
    } else if ( section == mPaintInfo.statusCol ) {
      if ( msg->status().isUnread() || msg->status().isNew() )
        setMsgStatus( KPIM::MessageStatus::statusRead() );
      else
        setMsgStatus( KPIM::MessageStatus::statusUnread() );
    }
  }
}
...
void KMHeaders::slotRMB()
{
  ...
  if ( item ) {
    ...
    if ( section == mPaintInfo.watchedIgnoredCol ) {
      mOwner->threadStatusMenu()->menu()->exec( QCursor::pos() );
      return;
    }
  }
...
bool KMHeaders::readSortOrder( bool set_selection, bool forceJumpToUnread )
{
    ...
    if (threaded && !unparented.isEmpty()) {
        ...
        if (mSubjThreading) {
            ...
            foreach ( SortCacheItem *item, toBeSubjThreaded ) {
