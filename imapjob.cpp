...
ImapJob::ImapJob( KMMessage *msg, JobType jt, KMFolderImap* folder,
    const QString &partSpecifier, const MessageViewer::AttachmentStrategy *as )
...
void ImapJob::slotGetMessageResult( KJob * job )
{
  ...
  if (job->error())
  {
    ...
    if ((*it).data.size() > 0)
    {
      kDebug() << "Retrieved part" << mPartSpecifier;
      if ( mPartSpecifier.isEmpty() ||
      ...
      kDebug() << "Got no data for" << mPartSpecifier;
      gotData = false;
      msg->setReadyToShow( true );
