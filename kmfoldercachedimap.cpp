...
void KMFolderCachedImap::readConfig()
{
  ...
  if ( mAnnotationFolderType != "FROMSERVER" ) {
    ...
    if ( !mAnnotationFolderType.isEmpty() &&
      ...
      kmkernel->iCalIface().setStorageFormat( folder(), KMailICalAdaptor::StorageXML );
    }
  }
...
void KMFolderCachedImap::slotMultiSetACLResult( KJob *job )
{
  ...
  if ( job->error() ) {
    ...
    kmkernel->iCalIface().addFolderChange( folder(), KMailICalAdaptor::ACL );
  }
...
void KMFolderCachedImap::updateAnnotationFolderType()
{
  ...
  if ( kmkernel->iCalIface().storageFormat( folder() ) == KMailICalAdaptor::StorageXML ) {
...
{
  QList<unsigned long> newMsgs = findNewMessages();
  kDebug(5006) << k_funcinfo << newMsgs << " of " << count() << endl;
  if ( newMsgs.isEmpty() )
    return 0;
  KMFolder *dest = 0;
...
{
  kDebug(5006) << k_funcinfo << folder << " " << root << endl;
  if ( root )
