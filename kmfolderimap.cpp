      ...
      if (static_cast<KMFolderImap*>(msgParent->storage())->account() == account())
      {
        ...
        if (folder() == msgParent)
        {
          ...
          for ( QStringList::Iterator it = sets.begin(); it != sets.end(); ++it )
          {
            ...
            if ( temp_msgs.isEmpty() ) kDebug() << "Wow! KMFolderImap::splitMessageList() returned an empty list!";
    ...
    const QString& myNamespace )
{
  ...
  while ( it.hasNext( ) ) {
    ...
    if ( !node->isDir() && !subfolderNames.contains(node->name()) ) {
      ...
      kDebug() << node->name() <<"in namespace" << myNamespace <<":" << isInNamespace;
      // ignore some cases
...
{
  kDebug() << folder()->name() << " finishMailCheck" << dbg << state;
  quiet( false );
  mContentState = state;
