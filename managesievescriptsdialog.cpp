...
void KMail::ManageSieveScriptsDialog::slotDeactivateScript() {
  Q3CheckListItem * item = qcli_cast( mListView->currentItem() );
...
void KMail::ManageSieveScriptsDialog::changeActiveScript( Q3CheckListItem * item, bool activate ) {
  if ( !item )
    return;
  if ( !mUrls.count( item ) )
    return;
