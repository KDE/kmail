...
void SaveAsFileJob::start()
{
    ...
    if (dlg->exec()) {
        ...
        if (dlg->selectedNameFilter() == QLatin1StringView("text/plain") || filename.endsWith(QLatin1String(".txt"))) {
            writer.setFormat("plaintext");
        } else if (dlg->selectedNameFilter() == QLatin1StringView("text/html") || filename.endsWith(QLatin1String(".html"))) {
            writer.setFormat("HTML");
        } else if (dlg->selectedNameFilter() == QLatin1StringView("application/vnd.oasis.opendocument.text") || filename.endsWith(QLatin1String(".odf"))) {
