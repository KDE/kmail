...
KMCommand::KMCommand(QWidget *parent, const Akonadi::Item &msg)
    : KMCommand(parent)
{
    if (msg.isValid() || msg.hasPayload<std::shared_ptr<KMime::Message>>()) {
        mMsgList.append(msg);
    }
}
...
KMEditMessageCommand::KMEditMessageCommand(QWidget *parent, const std::shared_ptr<KMime::Message> &msg)
    : KMCommand(parent)
    , mMessage(msg)
{
}
...
KMCommand::Result KMUseTemplateCommand::execute()
{
    ...
    std::shared_ptr<KMime::Message> newMsg(new KMime::Message);
...
{
    if (mUrl.isEmpty()) {
        mUrl = QFileDialog::getOpenFileUrl(parentWidget(), i18nc("@title:window", "Open Message"), QUrl(), u"%1 (*.mbox *.eml)"_s.arg(i18n("Message")));
    }
    if (mUrl.isEmpty()) {
        return Canceled;
    }
...
KMCommand::Result KMRedirectCommand::execute()
{
    ...
    for (const Akonadi::Item &item : lstItems) {
        ...
        const std::shared_ptr<KMime::Message> newMsg = factory.createRedirect(to, cc, bcc, transportId, fcc, identity);
        if (!newMsg) {
            return Failed;
        }
...
KMCommand::Result KMSaveAttachmentsCommand::execute()
{
    ...
    for (const Akonadi::Item &item : lstItems) {
        if (item.hasPayload<std::shared_ptr<KMime::Message>>()) {
            contentsToSave += item.payload<std::shared_ptr<KMime::Message>>()->attachments();
        } else {
...
KMCommand::Result KMResendMessageCommand::execute()
{
    ...
    factory.setIdentityManager(KMKernel::self()->identityManager());
    factory.setFolderIdentity(MailCommon::Util::folderIdentity(item));
    std::shared_ptr<KMime::Message> newMsg = factory.createResend();
    newMsg->contentType()->setCharset(MimeTreeParser::NodeHelper::charset(msg.get()));
...
{
    Akonadi::Item item = retrievedMessage();
    if (!item.isValid() || !item.hasPayload<std::shared_ptr<KMime::Message>>()) {
        return Failed;
    }
