...
void CollectionMailingListPage::init(const Akonadi::Collection &col)
{
    ...
    mHoldsMailingList = new QCheckBox(i18nc("@option:check", "Folder contains a single mailing list"), this);
...
void CollectionMailingListPage::save([[maybe_unused]] Akonadi::Collection &col)
{
    if (changed) {
        if (mFolder) {
...
void CollectionMailingListPage::fillMLFromWidgets()
{
    ...
    for (QStringList::ConstIterator it = oldList.constBegin(); it != end; ++it) {
        if (!(*it).startsWith(QLatin1StringView("http:")) && !(*it).startsWith("https:"_L1) && !(*it).startsWith("mailto:"_L1) && ((*it).contains(u'@'))) {
            listChanged = true;
            newList << u"mailto:"_s + *it;
        } else {
            newList << *it;
        }
    }
