...
#include "kmglobalns.h" //for text completion mode struct
...
void KMComposeWin::setupActions(int aCryptoMessageFormat)
{
  ...
  for ( int i=0; i < KMGlobalNS::numCompletionModes; ++i ) {
    if ( KMGlobalNS::completionModes[i].mode == mode )
...
void KMComposeWin::slotAttachPopupMenu(QListViewItem *, const QPoint &, int)
{
  if (!mAttachMenu)
  {
     ...
     mOpenId = mAttachMenu->insertItem(SmallIcon("fileopen"), i18n("to open", "Open"), this,
                             SLOT(slotAttachOpen()));
     mOpenWithId = mAttachMenu->insertItem(i18n("to open with", "Open with..."), this,
...
void KMComposeWin::slotMenuTextCompletion()
{
   ...
   composer.writeEntry( "Completion Mode", KMGlobalNS::completionModes[ nItem ].mode );
   config->sync();
