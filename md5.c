 ...
 *
 * Changes 6/96 by dl@hplyot.obspm.fr:
 * ----------------------------------
 * names modified to avoid clash with system libraries on SunOs,
 ...
 *
 * Revision 1.1  1996/05/09  13:11:28  dl
 * Initial revision
 *
 *
 */
...
#else
static void byteReverse _ANSI_ARGS_((unsigned char *buf, unsigned longs));
 ...
 * of bytes.
 */
void Bin_MD5Update(ctx, buf, len)
     struct Bin_MD5Context *ctx;
     ...
     unsigned len;
{
	...
	Bin_MD5Transform(ctx->buf, (uint32 *) ctx->in);
	buf += t;
	len -= t;
    }
 ...
 */
void Bin_MD5Final(digest,ctx)
     unsigned char digest[16]; struct Bin_MD5Context *ctx;
{
    unsigned count;
    unsigned char *p;
